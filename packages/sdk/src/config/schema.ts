...
export const TelemetryConfigSchema = z.object({
  /** OTLP/HTTP collector endpoint, e.g. an OTel Collector or vendor OTLP ingest host. */
   ...
   * Empty for a self-hosted collector. Managed backends that need an
   * ingestion key/token in a header go here, e.g. `{ 'x-api-key': '<key>' }`.
   * The env loader below parses these from OTEL_EXPORTER_OTLP_HEADERS in the
   * standard `k=v,k2=v2` form.
   */
...
export const DriftDetectionConfigSchema = z.object({
   ...
   * PrometheusMetricsSource (../drift/prometheus-source.ts) — Prometheus
   * itself, Grafana Mimir/Cortex/Thanos, or anything exposing /api/v1/query.
   * Not read at all if you inject your own MetricsQuerySource instead.
   */
  ...
  prometheusBearerToken: z.string().default(''),
});
...
export function loadDriftWatchConfigFromEnv(
  ...
  return DriftWatchConfigSchema.parse({
    ...
    driftDetection: {
      prometheusBaseUrl: env.PROMETHEUS_URL,
      prometheusBearerToken: env.PROMETHEUS_BEARER_TOKEN,
    },
  });
}
