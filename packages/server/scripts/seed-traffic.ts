 ...
 *
 * Reads BASE_URL / AUTH_TOKEN from packages/server/.env (loaded below) so the
 * command is the same across shells; override either by exporting it in your
 * own shell before running, if you need a one-off value.
 *
 * Usage:
 *   pnpm seed 40
 */
