...
*what to do about it*, either to notify a channel, or take a control action (pause,
...
evaluates your policy, and dispatches actions. It's turned **off** by default, and when
enabled starts in **shadow mode** so the full loop runs but takes no action and
...
policy fires when you expect. To rehearse the whole thing with no metrics
backend and no real traffic, add `DRIFT_DRY_RUN=1`, it drives the loop off fixtures.
...
Policy evaluation is a pure function, the same report + same policy always yields
...
2. A human clicks in any of the channels or resolves it in the console's Pending
...
**cost**: when your metrics dashboard shows token spend spiking, it downshifts from an expensive
...
MODEL_FALLBACK=qwen3.6-turbo  # the cheaper model to downshift to; becomes the switch target
AUTOPILOT_ENABLED=1
AUTOPILOT_MODE=enforce
AUTOPILOT_POLICIES='[{"when":{"tokenSpendDeltaPct":100},"do":["notify_slack","switch_model"]}]'
```

`MODEL_FALLBACK` must differ from `MODEL` — a fallback identical to the primary
is skipped when the registry is built, which would make `switch_model` a silent
no-op. Add more models by editing `model-client.ts`'s registry; set
...
`agent.model.switch` span (visible on the trace timeline) plus an
`agent.model.switches` counter. The detector reads that counter and
