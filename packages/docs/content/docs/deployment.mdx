---
title: Deployment
description: Docker as the primary path, docker-compose with Redis, running alongside self-hosted SigNoz, and the production checklist.
---
...
collector/query service ([signoz.md](/docs/signoz)) when you're ready.
...
[`docker-compose.signoz.yml`](https://github.com/codewithveek/drift-watch/blob/main/docker-compose.signoz.yml).
It's meant to be merged with SigNoz's own compose file (it references SigNoz's
`otel-collector` and `query-service` services, which only exist once merged):
...
- **Point telemetry at your real backend** — `OTEL_EXPORTER_OTLP_ENDPOINT` /
  ...
  drift detector's pull. ([signoz.md](/docs/signoz))
