...
It's a browser app served **by the server** at the root (`/`) — there's nothing
separate to deploy. The Docker image builds and bundles it automatically.

## Opening it

1. Browse to `http://<your-host>:4300/`.
2. Sign in with the administrator account created from `DW_USER` / `DW_PASSWORD`
   on the deployment's first boot. You'll be asked to choose your own password
   the first time.

That's the whole setup. The session lives in an `httpOnly` cookie, so no script
— including anything injected into the page — can read it, and every action you
take is attributed to you by name in the audit log.

The console polls the control-plane API every few
seconds; if a panel says data is unavailable, your session is
...
<Callout title="Local development">
  ...
  server on `:4300`. In production it's always the bundled `/`.
</Callout>
