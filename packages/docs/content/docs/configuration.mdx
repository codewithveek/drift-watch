...
| `AUTH_TOKEN` | `authToken` | `''` | Root bearer token: full access to every route and every agent, and the credential you mint the first API key with. Empty = local-only mode. Scoped [API keys](/docs/security#api-keys) work either way. See [security.md](/docs/security#authentication). |
