---
title: Configuration
description: Every environment variable and typed config field, for both the SDK and the reference server.
---
...
Schema: [`packages/sdk/src/config/schema.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/sdk/src/config/schema.ts).
...
| `OTEL_CAPTURE_PAYLOADS` | `capturePayloads` | `true` (set env to exactly `0` to disable) | Whether raw prompt text and tool-call inputs get attached to spans. See [security.md](/docs/security#telemetry-payload-capture). |
...
| Env var | Field | Default | Notes |
|---|---|---|---|
| `PROMETHEUS_URL` | `prometheusBaseUrl` | `http://localhost:9090` | Base URL of a Prometheus-compatible query API (Prometheus, Grafana Mimir/Cortex/Thanos, ...) — not the OTLP collector port. |
| `PROMETHEUS_BEARER_TOKEN` | `prometheusBearerToken` | `''` | Sent as an `Authorization: Bearer` header. Needed for managed backends (e.g. Grafana Cloud); leave empty for a local/self-hosted Prometheus with no auth in front of it. |

## Server config — `ServerConfig`

Schema: [`packages/server/src/config/server-config.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/server/src/config/server-config.ts).
...
| `DW_USER` | `adminUser` | `''` | Administrator account created on the **first** boot against an empty database, then ignored. Public sign-up is disabled, so without this there is no way into the console. |
| `DW_PASSWORD` | `adminPassword` | `''` | That admin's initial password; at least 12 characters. Changed on first sign-in. |
| `DW_AUTH_SECRET` | `authSecret` | `''` | Signs session cookies. Must be stable across restarts and identical across replicas. Required in production. |
| `DW_BASE_URL` | `baseUrl` | `''` | The origin users actually reach this deployment at. Must match the browser's address bar, or the session cookie is scoped to the wrong host and login silently fails. |
| `DATABASE_URL` | `databaseUrl` | `''` | Postgres connection URL. The durable backend, and a prerequisite for console login. Takes precedence over `REDIS_URL`. |
| `MAX_PROMPT_BYTES` | `maxPromptBytes` | `8192` | Per-request prompt size cap, independent of `BODY_LIMIT`. |
| `DRIFT_DRY_RUN` | `driftDryRun` | `false` (`'1'` → `true`) | `/drift` uses built-in fixtures instead of querying Prometheus. |
...
([`packages/sdk/src/config/schema.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/sdk/src/config/schema.ts)).
...
| `AUTOPILOT_ENABLED` | `autopilotEnabled` | `false` (`'1'` → `true`) | Runs the periodic scan loop and lets it act. With `0`, on-demand scans (`POST /drift/scan`, the console's "Scan now") still work but are forced into `shadow` mode — they report the intents they would have run without executing them. |
...
| `TOOL_CALL_APPROVAL_TIMEOUT_MS` | `toolCallApprovalTimeoutMs` | `120000` | How long a *pre-execution tool call* waits for a human before the default below applies. Deliberately much shorter than `AUTOPILOT_APPROVAL_TIMEOUT_MS`: that one is a background scan with nothing waiting on it, while this wait sits inside an in-flight `/run` request (and any reverse proxy in front of it, most defaulting to 30–60s). |
| `TOOL_CALL_APPROVAL_TIMEOUT_DECISION` | `toolCallApprovalTimeoutDecision` | `rejected` | Safe default when a tool-call approval times out. `rejected` denies the call — the model sees a tool error and can adapt; the run itself still completes. |
| `AUTOPILOT_SWITCH_MODEL_TO` | `switchModelTo` | `MODEL_FALLBACK` | Model id an approved `switch_model` action switches the agent to. Must be a key in `model-client.ts`'s registry. See [alerts-and-actions.md → Model switching](/docs/alerts-and-actions#model-switching-self-healing-cost--reliability). |
...
mutate the one shared store. See [alerts-and-actions.md](/docs/alerts-and-actions)
and [security.md](/docs/security#integration-webhook-authentication).
...
[`packages/sdk/src/autopilot/policy.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/sdk/src/autopilot/policy.ts).
...
to touch: [`packages/server/src/config/model-client.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/server/src/config/model-client.ts).
...
export const modelClient = qwenCloud(process.env.MODEL ?? 'qwen3.6-plus');
```

Set `QWEN_API_KEY` (and optionally `MODEL`) in your `.env`. To use a different
provider, see [server.md → Model client](/docs/server#the-model-client).
...
| `MODEL` | `qwen3.6-plus` | Primary model id — must be one your `QWEN_BASE_URL` endpoint actually serves. |
...
[`packages/server/src/tools.ts`](https://github.com/codewithveek/drift-watch/blob/main/packages/server/src/tools.ts)
...
See [`packages/server/.env.example`](https://github.com/codewithveek/drift-watch/blob/main/packages/server/.env.example)
for every variable above in one file, ready to copy to `packages/server/.env`.
