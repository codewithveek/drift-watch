---
title: Server
description: The reference Fastify service built on the SDK — HTTP surface, model client, and your own tools.
---

The server is a ready-to-run HTTP service built on the [SDK](/docs/sdk). It gives
...
Run it with Docker ([quickstart](/docs/quickstart)); this page is about what it
...
| `GET /drift` | Run drift detection now and return the two window stats + the model's verdict. Uses the configured Prometheus-compatible metrics source, or fixtures when `DRIFT_DRY_RUN=1`. |

Both require `Authorization: Bearer <api-key>` (or, if the deployment has no database and therefore no login,
a caller on your private network — see [security.md](/docs/security#authentication)).

```bash
curl -XPOST localhost:4300/run \
...
[alerts-and-actions.md](/docs/alerts-and-actions#channels).
...
<Callout title="What works and what doesn't">
...
- `DW_USER` / `DW_PASSWORD` / `DW_AUTH_SECRET` — the admin account and session
  secret. Required for any deployment you sign into.
- `DATABASE_URL` — required for anything other than local experimentation.
- `QWEN_API_KEY` (or your provider's key).
- `OTEL_EXPORTER_OTLP_ENDPOINT` / `PROMETHEUS_URL` + keys — to send/read telemetry
  ([prometheus.md](/docs/prometheus)).
- `TRUST_PROXY=1` — if behind a reverse proxy or load balancer.

See the [production checklist](/docs/deployment#production-checklist) before going
live.
