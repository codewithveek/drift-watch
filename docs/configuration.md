...
| `AUTOPILOT_COOLDOWN_MS` | `cooldownMs` | `300000` | Dedup window so the same action does not re-fire repeatedly during a burst of drift. |
...
| `AUTOPILOT_SWITCH_MODEL_TO` | `switchModelTo` | `MODEL_FALLBACK` | Model id an approved `switch_model` action switches the agent to. Must be a key in `model-client.ts`'s registry. See [alerts-and-actions.md → Model switching](./alerts-and-actions.md#model-switching-self-healing-cost--reliability). |
...
| `MODEL` | `qwen3.7-max` | Primary model id — e.g. `qwen3.7-max` / `qwen-plus` / `qwen-turbo`. |
