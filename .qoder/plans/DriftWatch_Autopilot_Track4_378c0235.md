...
- Rewrite `src/config/model-client.ts` to use `createOpenAI({ baseURL: QWEN_BASE_URL, apiKey: QWEN_API_KEY })` with `MODEL` default `glm-5.2` (OpenAI-compatible endpoint). Secrets read from `.env` only — never hardcoded.
